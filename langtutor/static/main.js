...
function startMessageWS() {
  ws = new WebSocket(`wss://${location.host}/ws`);
  ws.onclose = function () {
    ws = null
    setTimeout(startMessageWS, 2000)
  }
...
function startAudioWS() {
  audiows = new WebSocket(`wss://${location.host}/audiows`);
  audiows.onclose = function () {
    // connection closed, discard old websocket and create a new one in 5s
    audiows = null
    setTimeout(startAudioWS, 2000)
  }
  audiows.addEventListener('message', async function (event) {
    var audioData = event.data;
    await addAudiotoQueue(URL.createObjectURL(new Blob([audioData], { type: 'audio/mp3' })));
  });
...
function addMessage(sender, message_id, message, has_user_recording, is_language_learning, do_not_store, end) {
  ...
  if (message == "") {
    ...
    if (sender != 'user') {
      ...
      if (match) {
        var toAudio = match[1]; // Extract the substring up to the last occurrence
        requestAudio({ 'message_id': message_body.id, 'message': toAudio, 'play_existing': 1, 'end': message == "" })
        analysedText = match[2].trim(); // Extract the remaining text
